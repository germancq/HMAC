...
S_box = [0xE, 0xD, 0xB, 0x0, 0x2, 0x1, 0x4,
         0xF, 0x7, 0xA, 0x8, 0x5, 0x9, 0xC, 0x3, 0x6]
...
class Spongent:
    ...
    def initialize_lCounter(self):
        ...
        initial_state_options = {88: 0x5, 128: 0x7A,
                                 160: 0x45, 224: 0x01, 256: 0x9E}
        ...
        self.lCounter = LFSR.LFSR(
            size, self.initial_lCounter_state, feedback_coefficients
        )
    ...
    def iteration_permutation(self, state):
        reverse_counter = self.reverse_bits(
            self.lCounter.get_state(), self.lCounter.n)

        state = (
            state
            ^ (reverse_counter << (self.b - self.lCounter.n))
            ^ self.lCounter.get_state()
        )
...
if __name__ == "__main__":
    ...
    for t in range(1, 2):
        ...
        for i in range(0, j):
            ...
            spongent_state = spongent_impl.feed_data(
                data_chunk, spongent_state)
